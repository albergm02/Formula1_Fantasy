...
export const useLigasStore = defineStore('ligas', {
  ...
  actions: {
    async createLeague(leagueName) {
      ...
      const userEmail = authStore.usuarioGlobal.emailAuth

      if (hasReachedLeagueLimit(authStore.usuarioGlobal.ligasIds)) {
        return {
          success: false,
      ...
      try {
        ...
        authStore.usuarioGlobal.ligasIds.push(leagueId)
    ...
    async loadUserLeagues() {
      ...
      try {
        ...
        this.leagueDetails = leaguesData.filter((league) =>
          authStore.usuarioGlobal.ligasIds.includes(league.id),
        )
      } catch (error) {
    ...
    async joinLeague(inviteCode) {
      ...
      try {
        ...
        if (authStore.usuarioGlobal.ligasIds.includes(leagueId)) {
