  ...
  <div class="mt-20 min-h-screen w-full p-4 pb-32 relative font-sans">
    <div class="mx-auto w-full max-w-5xl flex flex-col gap-8">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex flex-col justify-center">
              ...
              <div class="flex items-center gap-2 mt-1">
                ...
                <span class="text-xs text-[#10b981] font-bold tracking-wide">{{ fantasyStore.usuario.presupuesto
                }}M</span>
              </div>
            </div>
          </div>
          <Button @click="cerrarSesion" icon="pi pi-sign-out"
            class="!w-10 !h-10 !bg-[#2e2e38] !text-[#8a8a9d] !border-none hover:!bg-[#e10600] hover:!text-white" />
        </div>
      </header>
      ...
      <section>
        ...
        <Card v-if="fantasyStore.garaje.coche"
          class="!bg-[#15151e] !border !border-[#2e2e38] !shadow-none w-full md:w-1/2 overflow-hidden">
          <template #header>
            <div class="relative aspect-[4/3] bg-[#2e2e38]/30">
              ...
              <div
                class="absolute top-2 left-2 bg-[#15151e]/80 backdrop-blur-sm px-2 py-1 rounded text-xs font-black text-white">
                {{ fantasyStore.garaje.coche.nombre }} <span class="text-[#10b981] ml-1">{{
                  fantasyStore.garaje.coche.precio }}M</span>
              </div>
            </div>
          </template>
          <template #footer>
            <Button @click="confirmarVenta(fantasyStore.garaje.coche, 'coche')" label="VENDER" icon="pi pi-shopping-bag"
              class="!w-full !bg-transparent !text-[#8a8a9d] !border !border-[#2e2e38] hover:!border-red-500 hover:!text-red-500 !text-xs !font-black" />
          </template>
        </Card>

        <div v-else
          class="flex flex-col items-center justify-center p-8 border-2 border-dashed border-[#2e2e38] rounded-xl text-[#8a8a9d]">
      ...
      <section>
        ...
        <div v-if="fantasyStore.garaje.pilotos.length > 0" class="grid grid-cols-2 md:grid-cols-3 gap-4">
          <Card v-for="piloto in fantasyStore.garaje.pilotos" :key="piloto.idInstancia"
            class="!bg-[#15151e] !border !border-[#2e2e38] !shadow-none overflow-hidden flex flex-col h-full">
            <template #header>
              <div class="relative aspect-[3/4] bg-[#2e2e38]/30">
                <img :src="piloto.imagen" class="absolute inset-0 w-full h-full object-cover" />
                <div
                  class="absolute bottom-2 left-2 right-2 bg-[#15151e]/80 backdrop-blur-sm px-2 py-1 rounded text-xs font-black text-white truncate text-center">
                  {{ piloto.nombre }}
                </div>
              </div>
            </template>
            <template #footer>
              <Button @click="confirmarVenta(piloto, 'piloto')" label="VENDER"
                class="!w-full !p-2 !bg-transparent !text-[#8a8a9d] !border !border-[#2e2e38] hover:!border-red-500 hover:!text-red-500 !text-xs !font-black" />
            </template>
          </Card>
        </div>
      ...
      <section class="mb-32">
        ...
        <div v-if="fantasyStore.garaje.potenciadores.length > 0" class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card v-for="pieza in fantasyStore.garaje.potenciadores" :key="pieza.idInstancia"
            class="!bg-[#15151e] !border !shadow-none overflow-hidden"
            :class="pieza.equipado ? '!border-[#e10600]' : '!border-[#2e2e38]'">
            <template #header>
