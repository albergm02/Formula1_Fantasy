...
<template>
  <div :style="{ height: currentHeight + 'px', scrollbarWidth: 'none' }" class="flex items-center mx-2 max-w-full">
    <div @mousemove="handleMouseMove" @mouseleave="handleMouseLeave"
      :class="`${props.className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-end w-fit gap-4 border-neutral-700 border-2 pb-2 px-4`"
  ...
  render() {
    ...
    return h(
      'div',
      {
        ...
        class: `relative cursor-pointer inline-flex items-center justify-center bg-[#111] border-neutral-700 border-2 shadow-md ${this.className}`,
        tabindex: 0,
        role: 'button',
        'aria-haspopup': 'true'
      },
...
const DockLabel = defineComponent({
  ...
  render() {
    return h(
      'div',
      {
        class: `${this.className} absolute -top-8 left-1/2 w-fit whitespace-pre border border-neutral-700 bg-[#111] px-2 py-0.5 text-xs text-white transition-all duration-200`,
        role: 'tooltip',
        style: {
