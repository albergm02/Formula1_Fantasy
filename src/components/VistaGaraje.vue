...
<template>
  <div class="flex flex-col gap-6" :class="modoRival ? 'p-4 max-w-lg mx-auto' : 'w-full max-w-lg mx-auto mt-4 mb-24 px-3'">
        ...
        <article v-for="coche in coches" :key="coche.instancia_id" class="flex flex-col bg-[#121218]">
          ...
          <div class="flex flex-col gap-2 px-2 py-2 items-center">
            ...
            <div v-if="!modoRival" class="grid grid-cols-3 gap-1.5 w-full">
              ...
              <Button
                label="Proteger"
                @click="abrirDialogoProteccion(coche)"
                size="small"
                class="!bg-[#1A1A1F] !border-[#D4A843]/40"
                :pt="{ label: { class: 'text-[10px] font-black uppercase tracking-wide text-[#D4A843]' } }"
              />
              <Button
                :label="`Vender ${calcularValorReventa(coche.precio).toFixed(2)}M`"
                @click="confirmarVentaCoche(coche)"
                size="small"
    ...
    <section>
      ...
      <div v-if="pilotos.length > 0" class="grid grid-cols-1 gap-3">
        <article v-for="piloto in pilotos" :key="piloto.instancia_id" class="flex flex-col bg-[#121218] border border-zinc-800">
          ...
          <div class="flex flex-col gap-2 px-3 py-2 border-t border-zinc-800/70">
            ...
            <div v-if="!modoRival" class="grid grid-cols-3 gap-1.5">
              ...
              <Button
                label="Proteger"
                @click="abrirDialogoProteccion(piloto)"
                size="small"
                class="!bg-[#1A1A1F] !border-[#D4A843]/40"
                :pt="{ label: { class: 'text-[10px] font-black uppercase tracking-wide text-[#D4A843]' } }"
              />
              <Button
                :label="`Despedir ${calcularValorReventa(piloto.precio).toFixed(2)}M`"
